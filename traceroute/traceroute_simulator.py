...
def Traceroute(dest_name):  
    ...
    while True:
        ...
        sending_socket.sendto("testing".encode(), (dest_name, port)) #To specify the sending socket where and what to send
        ...
        if current_hop is not None: #if the conversion was successfull add it to current address
            curr_host = "{} ({})".format(curr_name, current_hop) # "%s (%s)" % (curr_name, current_hop)
        else:
